...
Item {
    ...
    property real initX: Math.max((windowData?.at[0] - (monitorData?.x ?? 0) - monitorData?.reserved[0]) * widthRatio * root.scale, 0) + xOffset
    property real initY: Math.max((windowData?.at[1] - (monitorData?.y ?? 0) - monitorData?.reserved[1]) * heightRatio * root.scale, 0) + yOffset
    ...
    property bool centerIcons: WorkspaceOverviewSettings.centerIcons
    ...
    property bool compactMode: 13 * 4 > targetWindowHeight || 13 * 4 > targetWindowWidth

    property bool indicateXWayland: windowData?.xwayland ?? false
    property bool isOnActiveWorkspace: false
    ...
    clip: !layer.enabled
    layer.enabled: !root.Drag.active
    ...
    ScreencopyView {
        id: windowPreview
        anchors.fill: parent
        captureSource: WorkspaceOverviewState.overviewOpen ? root.toplevel : null
        live: WorkspaceOverviewState.overviewOpen && root.isOnActiveWorkspace
        constraintSize: Qt.size(WorkspaceOverviewSettings.livePreviewMaxWidth, WorkspaceOverviewSettings.livePreviewMaxHeight)
        ...
        Rectangle {
            ...
            bottomRightRadius: root.bottomRightRadius
            bottomLeftRadius: root.bottomLeftRadius
            color: pressed ? WorkspaceOverviewSettings.windowOverlayPressColor :
                hovered ? WorkspaceOverviewSettings.windowOverlayHoverColor :
                WorkspaceOverviewSettings.windowOverlayColor
            border.color: WorkspaceOverviewSettings.windowBorderColor
            border.width: 1
        }
